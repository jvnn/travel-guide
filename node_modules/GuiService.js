...
function GuiService() {
  ...
  function getSmallSummary(item, killMarker, saveChanges) {
    ...
    var texts = [item['name'], item['description']];
    var callbacks = [idDelete, idConfirmDelete, idSave];
    var conditions = [item['name'] != null, item['description'] != null];
    return create('html/components/small-summary.html', [], texts, callbacks, conditions);
  }
  ...
  function create(filename, ids, texts, callbackIds, conditions) {
    ...
    for (var idx in conditions) {
      if (!conditions[idx]) {
        var startIdx = html.indexOf('@!if');
        var endIdx = html.indexOf('@!endif') + '@!endif'.length;
